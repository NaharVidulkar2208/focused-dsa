...
export const Route = createFileRoute("/harry/cpp")({
  ...
  component: HarryCppLayout,
});

function loadProgress(): Set<string> {
  try {
    const raw = localStorage.getItem(HARRY_CPP_PROGRESS_KEY);
    return raw ? new Set(JSON.parse(raw) as string[]) : new Set();
  } catch { return new Set(); }
}
function saveProgress(set: Set<string>) {
  try { localStorage.setItem(HARRY_CPP_PROGRESS_KEY, JSON.stringify([...set])); } catch {}
}
      ...
      <header className="flex h-14 shrink-0 items-center justify-between gap-3 border-b border-white/8 bg-zinc-950/95 px-4 backdrop-blur">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <div className={`grid h-7 w-7 shrink-0 place-items-center rounded-lg ring-1 ${HARRY_CPP_THEME.iconBg} ${HARRY_CPP_THEME.iconRing}`}>
              <Sparkles className={`h-3.5 w-3.5 ${HARRY_CPP_THEME.iconColor}`} />
            </div>
            <div className="hidden sm:block">
              <span className="text-sm font-semibold text-zinc-100">C++ + DSA</span>
          ...
          {!isEmpty && (
            <div className="hidden sm:flex items-center gap-2">
              <div className="h-1.5 w-24 overflow-hidden rounded-full bg-white/8">
                <div
                  className={`h-full rounded-full bg-gradient-to-r ${HARRY_CPP_THEME.progressBar} transition-all`}
                  style={{ width: `${pct}%` }}
                />
              </div>
      ...
      {isEmpty ? (
        <div className="flex flex-1 items-center justify-center px-6">
          <div className="max-w-sm text-center">
            <div className={`mx-auto mb-4 grid h-12 w-12 place-items-center rounded-2xl ring-1 ${HARRY_CPP_THEME.emptyIconBg} ${HARRY_CPP_THEME.emptyIconRing}`}>
              <Sparkles className={`h-5 w-5 ${HARRY_CPP_THEME.emptyIconColor}`} />
            </div>
            <h2 className="text-base font-semibold text-zinc-200">C++ lectures coming soon</h2>
            <Link
              to="/harry"
              className={`mt-5 inline-flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-medium transition ${HARRY_CPP_THEME.emptyBtn}`}
            >
              ...
              <aside className="relative z-10 flex w-[85vw] max-w-xs flex-col bg-zinc-900 shadow-2xl">
                <div className="flex items-center justify-between border-b border-white/8 px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold">C++ + DSA</span>
                  </div>
...
function LectureView({
  ...
  useEffect(() => {
    try { localStorage.setItem(HARRY_CPP_LAST_KEY, lecture.id); } catch {}
  }, [lecture.id]);
  ...
  const handleToggle = () => {
    ...
    if (!isDone && next) {
      setTimeout(() => navigate({ to: "/harry/cpp/$lectureId", params: { lectureId: next.id } }), 600);
    }
  };
    ...
    <div className="flex flex-col">
      ...
      <div className="mx-auto w-full max-w-3xl px-4 py-5">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition ${
              isDone
                ...
                : HARRY_CPP_THEME.markCompleteBtn
            }`}
          >
            {isDone
          ...
          {prev && (
            <button
              onClick={() => navigate({ to: "/harry/cpp/$lectureId", params: { lectureId: prev.id } })}
          ...
          {next && (
            <button
              onClick={() => navigate({ to: "/harry/cpp/$lectureId", params: { lectureId: next.id } })}
              className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs text-zinc-400 hover:bg-white/8 hover:text-zinc-200 transition"
            >
