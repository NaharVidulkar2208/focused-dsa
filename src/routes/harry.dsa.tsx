...
import { HARRY_DSA_THEME } from "@/lib/course-theme";
import {
...
function HarryDsaLayout() {
  ...
  useEffect(() => {
    ...
    const target = (last && HARRY_DSA_LECTURES.find((l) => l.id === last)) ?? HARRY_DSA_LECTURES[0];
      ...
      <header className="flex h-14 shrink-0 items-center justify-between gap-3 border-b border-white/8 bg-zinc-950/95 px-4 backdrop-blur">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <div className={`grid h-7 w-7 shrink-0 place-items-center rounded-lg ring-1 ${HARRY_DSA_THEME.iconBg} ${HARRY_DSA_THEME.iconRing}`}>
              <Sparkles className={`h-3.5 w-3.5 ${HARRY_DSA_THEME.iconColor}`} />
            </div>
            <div className="hidden sm:block">
              <span className="text-sm font-semibold text-zinc-100">DSA Course</span>
          ...
          {!isEmpty && (
            <div className="hidden sm:flex items-center gap-2">
              <div className="h-1.5 w-24 overflow-hidden rounded-full bg-white/8">
                <div
                  className={`h-full rounded-full bg-gradient-to-r ${HARRY_DSA_THEME.progressBar} transition-all`}
                  style={{ width: `${pct}%` }}
                />
              </div>
      ...
      {isEmpty ? (
        <div className="flex flex-1 items-center justify-center px-6">
          <div className="max-w-sm text-center">
            <div className={`mx-auto mb-4 grid h-12 w-12 place-items-center rounded-2xl ring-1 ${HARRY_DSA_THEME.emptyIconBg} ${HARRY_DSA_THEME.emptyIconRing}`}>
              <Sparkles className={`h-5 w-5 ${HARRY_DSA_THEME.emptyIconColor}`} />
            </div>
            ...
            <Link
              to="/harry"
              className={`mt-5 inline-flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-medium transition ${HARRY_DSA_THEME.emptyBtn}`}
            >
        ...
        <div className="flex min-h-0 flex-1">
          <aside className="hidden md:flex w-72 shrink-0 flex-col border-r border-white/8 bg-zinc-900/50">
            <CourseSidebar
              topics={HARRY_DSA_TOPICS}
              byTopic={HARRY_DSA_BY_TOPIC}
              activeLectureId={params.lectureId}
              completed={completed}
              onNavigate={(id) => navigate({ to: "/harry/dsa/$lectureId", params: { lectureId: id } })}
              theme={HARRY_DSA_THEME}
            />
          </aside>
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition ${
              isDone
                ...
                : HARRY_DSA_THEME.markCompleteBtn
            }`}
          >
            {isDone
